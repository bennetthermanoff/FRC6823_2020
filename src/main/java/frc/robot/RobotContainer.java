...
public class RobotContainer {
    ...
    private void configureButtonBindings() {
        // This is the swerve "STOP" button, it will tell the swerve to immidiately stop
        // and set all motors to brake.
        joystickHandler.button(9)
                .whileHeld(() -> swerveDriveSubsystem.drive(0, 0, 0), swerveDriveSubsystem);

        // press button 12 to set the swerve just forward, this is for calibration
        // purposes
        joystickHandler.button(12)
                .whileHeld(() -> swerveDriveSubsystem.drive(0.2, 0, 0), swerveDriveSubsystem);

        // this will set the current orientation to be "forward" for field drive
        joystickHandler.button(8)
                .whenPressed(() -> fieldSpaceDriveCommand.zero());

        // holding 10 will enable field space drive, instead of robot space
        joystickHandler.button(10)
                .whileHeld(fieldSpaceDriveCommand);

        joystickHandler.button(3)
                .whileHeld(() -> {
                    double[] autoAim = limeLight.goToPolar(
                            Robot.PREFS.getDouble("PolarDistance", 50),
        ...
        joystickHandler.button(4)
                .whileHeld(() -> {
                    double[] autoAim = limeLight.goTo(
                            Robot.PREFS.getDouble("CY", 50),
                            Robot.PREFS.getDouble("CX", 0));
                    swerveDriveSubsystem.drive(autoAim[2] * .65, autoAim[1] * .1, autoAim[0] * .3);
                }, swerveDriveSubsystem);

        joystickHandler.button(6)
                .whileHeld(() -> {
